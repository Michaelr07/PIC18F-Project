...
extern char buffer[31];                             // external variables declared
extern char *nbr;                                   //
extern char *txt;                                   //
extern char tempC[];                                //
extern char tempF[];                                //
...
void Initialize_Screen(void)                        // Initialize Screen function
{ 
    LCD_Reset();                                    // Screen reset
    TFT_GreenTab_Initialize();                      // call TFT green tab initialize
...
void Update_Screen(void)                            // Update screen function
{   
    if ((DS1621_tempC & 0x80) == 0x80)              // If temperature C is equal to 0x80
    {
        tempC[0] = '-';                             // -
    ...
    if (DS1621_tempF < 0)                           // if temperature F is less than 0
    {
        tempF[0] = '-';                             //
        int neg_tempF = -DS1621_tempF;              // make temp F negative
    ...
    if (ALARMEN == 1)                               // If Alarm Enable is 1
    {   
        strcpy(Alarm_SW_Txt, "ON ");                // Write 'ON' on screen
    }
    else 
    {
        strcpy(Alarm_SW_Txt, "OFF");                // Write 'OFF' on screen
    }
    ...
    if (FAN == 1)                                   // If fan is 1
    {
        strcpy(Fan_SW_Txt, "ON ");                  // Write 'ON"
    }
    else 
    {
        strcpy(Fan_SW_Txt, "OFF");                  // Write 'OFF'
    }
    
    if (RTC_ALARM_NOT == 0) RTC_ALARM_Txt[0] = '1'; // is RTC is 0
    else RTC_ALARM_Txt[0] = '0';                    // 
       
    
    DC_Txt[0] = duty_cycle/100  + '0';              // ones place
    DC_Txt[1] = (duty_cycle%100)/10 +'0';           // tens place
    DC_Txt[2] = duty_cycle%10 + '0';                // hundreds place     
    
    Volt_Txt[0] = ((int)   volt ) + '0';            // ones place
    Volt_Txt[2] = ((int)  (volt*10)%10) + '0';      // tens place
    Volt_Txt[3] = ((int)  (volt*100)%10) + '0';     // hundreds place   
    
    RPM_Txt[0] = rpm/1000  + '0';                   // ones place
    RPM_Txt[1] = (rpm%1000)/100 + '0';              // tens place  
    RPM_Txt[2] = (rpm%100)/10 + '0';                // hundreds place    
    RPM_Txt[3] = rpm%10 + '0';                      // thousands place       
